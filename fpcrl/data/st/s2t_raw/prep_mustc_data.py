...
def process(args):
    ...
    for lang in MUSTC.LANGUAGES:
        ...
        save_root = Path(args.spm_data).absolute() / f"en-{lang}"
        spminfo_root = Path(args.spm_info).absolute() / f"en-{lang}"
        ...
        audio_root = save_root / ("flac" if args.use_audio_input else "fbank80")
        # audio_root.mkdir(exist_ok=True)
        audio_root.mkdir(parents=True, exist_ok=True)
        spminfo_root.mkdir(parents=True, exist_ok=True)
        ...
        zip_path = save_root / f"{audio_root.name}.zip"
        if not use_zipped:
        ...
        with NamedTemporaryFile(mode="w") as f:
            ...
            gen_vocab(
                ...
                spminfo_root / spm_filename_prefix,
                args.vocab_type,
                args.vocab_size,
            )
        ...
        if args.use_audio_input:
            ...
            gen_config_yaml(
                ...
                gcmvn_path=(
                    save_root / "gcmvn.npz" if args.cmvn_type == "global"
                    else None
                ),
            )
...
def main():
    ...
    parser.add_argument("--spm-info", "-s", required=True, type=str)
    ...
    parser.add_argument("--min_n_frames", default=0, type=int)
