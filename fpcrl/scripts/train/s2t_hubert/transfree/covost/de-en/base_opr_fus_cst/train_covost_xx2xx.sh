...
export ARCH=s2t_hubert_opr_post
export CRITERION=label_smoothed_cross_entropy_opr
export FUS_WP=5000
export CST_WP=5000
...
export LOG_DIR=${ROOT}/scripts/${TYPE}/${TASK}/${DATA}/${PAIR}/${BRAN}/${BRAN}_log
...
fairseq-train ${COVOST_ROOT} \
	...
	--use-fus --fus-warmup ${FUS_WP} --use-cst --cst-warmup ${CST_WP} \
	--hubert-model-path ${PREMODEL}/hubert_base_ls960.pt \
	--config-yaml ${SPM_INFO}/config_st_${LANG}_en.yaml --train-subset train_st_${LANG}_en_raw --valid-subset dev_st_${LANG}_en_raw \
	--max-tokens 2000000 --max-source-positions 900000 --max-target-positions 1024 --batch-size 32 \
	--max-epoch 96 --max-update 80000 --num-workers 4 \
