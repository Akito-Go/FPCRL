...
class S2TTransformerModelW2V2(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--freeze-w2v2",
            action="store_true",
            help="if we want to freeze the w2v2 features"
        )
...
class S2TTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary=None, embed_tokens=None):
        ...
        self.w2v2_model, self.w2v2_args = self.load_w2v2(self.w2v2_model_path)
        self.freeze_w2v2 = getattr(args, "freeze_w2v2", False)
        if self.freeze_w2v2:
        ...
        if args.conv_kernel_sizes:
            self.subsampler = Conv1dSubsampler(
                self.w2v2_args.model.encoder_embed_dim,
                args.conv_channels,
    ...
    def load_w2v2(self, ckpt):
        ckpt = checkpoint_utils.load_checkpoint_to_cpu(ckpt)
        w2v2_args = ckpt["cfg"]
        w2v2_task = tasks.setup_task(w2v2_args.task)
        if "task_state" in ckpt:
            w2v2_task.load_state_dict(ckpt["task_state"])
        model = w2v2_task.build_model(w2v2_args.model)
        model.load_state_dict(ckpt["model"], strict=True)
        model.remove_pretraining_modules()
        return model, w2v2_args

    def get_w2v2_features(self, audio, audio_lengths):
        ...
        # x, padding_mask = self.w2v2_model.extract_features(**w2v2_args)
        output = self.w2v2_model.extract_features(**w2v2_args)
        x, padding_mask = output["x"], output["padding_mask"]
    ...
    def _forward(self, src_tokens, src_lengths, mode="st", return_all_hiddens=False):
        if mode == "st":
            x, encoder_padding_mask, input_lengths = self.get_w2v2_features(src_tokens, src_lengths)
...
@register_model_architecture(model_name="s2t_w2v2", arch_name="s2t_w2v2_pre")
def base_architecture(args):
    args.encoder_freezing_updates = getattr(args, "encoder_freezing_updates", 0)

    # Wav2vec 2.0
    args.w2v2_model_path = getattr(args, "w2v2_model_path", "")
    args.freeze_w2v2 = getattr(args, "freeze_w2v2", False)
...
@register_model_architecture(model_name="s2t_w2v2", arch_name="s2t_w2v2_post")
def s2t_w2v2_post(args):
