...
@register_model("s2t_hubert_mtl_opr")
class S2TTransformerModelHuBERT(FairseqEncoderDecoderModel):
    """
    ...
    def add_args(parser):
        ...
        # extra encoders for purification
        parser.add_argument(
            "--ful-encoder-layers", type=int, metavar="N", help="num full information encoder layers"
        )
        parser.add_argument(
            "--irr-encoder-layers", type=int, metavar="N", help="num irrelevant information encoder layers"
        )
...
class S2TTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary=None, embed_tokens=None):
        ...
        self.ful_transformer_layers = nn.ModuleList(
            [TransformerEncoderLayer(args) for _ in range(args.ful_encoder_layers)]
        )
        self.irr_transformer_layers = nn.ModuleList(
            [TransformerEncoderLayer(args) for _ in range(args.irr_encoder_layers)]
        )
    ...
    @staticmethod
    def OrthogonalProjection(H_ful, H_irr):
        """
        Project H_ful onto H_irr and remove redundancy. The shapes are [T, B, C]
        """

        # Step 1: Compute H_ful · H_irr, maintaining [T, B] dimensions
        dot_ful_irr = torch.sum(H_ful * H_irr, dim=-1, keepdim=True)  # [T, B, 1]

        # Step 2: Compute the norm |H_irr|, keeping the dimensions [T, B, 1]
        norm_irr = torch.norm(H_irr, p=2, dim=-1, keepdim=True)  # [T, B, 1]

        # Step 3: Calculate projection
        H_ful_irr = (dot_ful_irr / norm_irr) * (H_irr / norm_irr)  # [T, B, C]

        # Step 4: Remove redundant parts of the projection
        H_ful_pur = H_ful - H_ful_irr

        return H_ful_pur, H_ful_irr

    @staticmethod
    def FeaFuse(p, s, p_star):
        T, B, C = p.shape

        # Define the weight matrix W and bias b (the concatenated dimension is 2C -> C)
        W = nn.Parameter(torch.rand(2 * C, C)).to(p.dtype).to(p.device)  # [2C, C]
        b = nn.Parameter(torch.zeros(C)).to(p.dtype).to(p.device)  # [C]

        # Concatenate p and s, and do it on the last dimension (channel dimension)
        concat_features = torch.cat([p, s], dim=-1)  # [T, B, 2C]

        # Calculate h
        sigmoid = nn.Sigmoid()
        h = sigmoid(torch.einsum("tbc,cd->tbd", concat_features, W) + b)  # [T, B, C]

        # Calculate s'
        s_prime = h * p_star + s  # [T, B, C]

        return s_prime

    @staticmethod
    def add_gaussian_noise(x):
        snr_values = [5, 10, 20, 50, float('inf')]

        # Randomly choose the signal-to-noise ratio ε
        snr = random.choice(torch.tensor(snr_values))

        # Calculate the standard deviation of a signal
        signal_std = torch.std(x, dim=-1, keepdim=True)

        # For ε = +∞, no noise is added
        if snr == float('inf'):
            return x

        # Calculate the standard deviation of the noise
        noise_std = signal_std / torch.sqrt(snr)

        # Generate noise of the same shape as the input
    ...
    def _forward(self, src_tokens, src_lengths, mode="st", return_all_hiddens=False):
        if mode == "st":
            if self.training:
                src_tokens = self.mask_src(src_tokens)
            ...
            y_src_tokens = self.add_gaussian_noise(src_tokens)
            y, y_encoder_padding_mask, y_input_lengths = self.get_hubert_features(y_src_tokens, src_lengths)
            if self.subsampler is not None:
                y, y_input_lengths = self.subsampler(y, y_input_lengths)
                y_encoder_padding_mask = lengths_to_padding_mask(y_input_lengths)
        ...
        if mode == "st":
            ...
            x_ful = x_irr = x
            y_ful = y_irr = y
            for ful_layer in self.ful_transformer_layers:
                x_ful = ful_layer(x_ful, encoder_padding_mask)
                y_ful = ful_layer(y_ful, y_encoder_padding_mask)
            for irr_layer in self.irr_transformer_layers:
                x_irr = irr_layer(x_irr, encoder_padding_mask)
                y_irr = irr_layer(y_irr, y_encoder_padding_mask)
            x_ful_pur, x_ful_irr = self.OrthogonalProjection(x_ful, x_irr)
            y_ful_pur, y_ful_irr = self.OrthogonalProjection(y_ful, y_irr)
            x_irr_fus = self.FeaFuse(x_ful, x_irr, x_ful_irr)
            x = x_ful_pur
        ...
        return {
            ...
            "encoder_fus_out": [x_irr, x_irr_fus] if mode == "st" else None,  # T x B x C
            "encoder_cst_out": [x_ful_pur, y_ful_pur] if mode == "st" else None,  # T x B x C
            "encoder_padding_mask": [encoder_padding_mask, y_encoder_padding_mask] if mode == "st" else [encoder_padding_mask],
...
@register_model_architecture(model_name="s2t_hubert_mtl_opr", arch_name="s2t_hubert_mtl_opr_pre")
def base_architecture(args):
    ...
    # extra encoders
    args.ful_encoder_layers = getattr(args, "ful_encoder_layers", 1)
    args.irr_encoder_layers = getattr(args, "irr_encoder_layers", 1)
...
@register_model_architecture(model_name="s2t_hubert_mtl_opr", arch_name="s2t_hubert_mtl_opr_post")
def s2t_hubert_mtl_opr_post(args):
