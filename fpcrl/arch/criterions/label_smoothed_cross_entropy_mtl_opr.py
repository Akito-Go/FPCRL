...
def _compute_cst_loss(x_all_pur, y_all_pur, padding_mask, y_padding_mask):
    x_all_pur_mean = get_sequence_representation(x_all_pur, padding_mask)
    y_all_pur_mean = get_sequence_representation(y_all_pur, y_padding_mask)
    cst_loss = torch.norm(x_all_pur_mean - y_all_pur_mean, p=2)
    return cst_loss


@register_criterion(
    "label_smoothed_cross_entropy_mtl_opr", dataclass=LabelSmoothedCrossEntropyCriterionConfig
)
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward_st(self, model, sample, reduce):
        ...
        return loss, audio_output, target
    ...
    def forward_mt(self, model, sample, reduce):
        ...
        return loss, text_output, target
    ...
    def forward(self, model, sample, reduce=True):
        ...
        jsd_loss, fus_loss, cst_loss = torch.Tensor([0]).cuda(), torch.Tensor([0]).cuda(), torch.Tensor([0]).cuda()
        st_size, mt_size, jsd_size, fus_size, cst_size = 0, 0, 0, 0, 0

        if self.training:
            st_loss, st_output, st_target = self.forward_st(model, sample, reduce)
            mt_loss, mt_output, mt_target = self.forward_mt(model, sample, reduce)
            ...
            if self.use_jsd:
                jsd_loss = self.compute_jsd_loss(st_lprobs, mt_lprobs, st_target, mt_target, self.padding_idx)
                loss = loss + jsd_loss
            if self.use_fus:
                fus_weight = min((model.num_updates / self.fus_warmup), 1)
                if fus_weight > 0:
                    fus_loss = self.compute_fus_loss(st_output)
                loss = loss + fus_loss * fus_weight
            if self.use_cst:
                cst_weight = min((model.num_updates / self.cst_warmup), 1)
                if cst_weight > 0:
                    cst_loss = self.compute_cst_loss(st_output)
                loss = loss + cst_loss * cst_weight
            else:
                pass
            st_size = mt_size = jsd_size = fus_size = cst_size = sample_size = sample["ntokens"]
        else:
            st_loss, st_output, _ = self.forward_st(model, sample, reduce)
            loss = st_loss
    ...
    def compute_fus_loss(self, net_output):
        if net_output[1]["encoder_out"]["encoder_fus_out"] is not None:
    ...
    def compute_cst_loss(self, net_output):
        if net_output[1]["encoder_out"]["encoder_cst_out"] is not None:
            x_all_pur = net_output[1]["encoder_out"]["encoder_cst_out"][0]
            y_all_pur = net_output[1]["encoder_out"]["encoder_cst_out"][-1]
            padding_mask = net_output[1]["encoder_out"]["encoder_padding_mask"][0]
            y_padding_mask = net_output[1]["encoder_out"]["encoder_padding_mask"][-1]
            cst_loss = _compute_cst_loss(x_all_pur, y_all_pur, padding_mask, y_padding_mask)
        else:
            cst_loss = 0
        return cst_loss
    ...
    def reduce_metrics(cls, logging_outputs) -> None:
        ...
        jsd_sample_size = sum(log.get("jsd_sample_size", 0) for log in logging_outputs)
        ...
        metrics.log_scalar(
            "jsd_loss", jsd_loss_sum / jsd_sample_size / math.log(2) if jsd_sample_size != 0 else 0, jsd_sample_size,
            round=3
        )
        metrics.log_scalar(
            "fus_loss", fus_loss_sum / fus_sample_size / math.log(2) if fus_sample_size != 0 else 0, fus_sample_size,
            round=3
        )
        metrics.log_scalar(
            "cst_loss", cst_loss_sum / cst_sample_size / math.log(2) if cst_sample_size != 0 else 0, cst_sample_size,
            round=3
        )
